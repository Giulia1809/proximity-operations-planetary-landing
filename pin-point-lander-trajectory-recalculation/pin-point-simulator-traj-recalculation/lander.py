...
#Define Lander class - this class should also load .urdf files for terrain and lander mesh configuration:
class Lander:
  ...
  def reset(self):
    ...
    #self.terrainObject2 = self._p.loadURDF("plane_background.urdf") #uncomment only for Titan env
    ...
    self.force1 = [0., 0., self.force1Diff + self.baseForce] #along z
    self.force2 = [0., self.force2Diff + self.baseForce, 0.] #along y
    self.force3 = [self.force3Diff + self.baseForce, 0., 0.] #along x
    
    self._p.applyExternalForce(lander, -1, self.force1, [0, 0, 0], flags=self._p.LINK_FRAME)
    self._p.applyExternalForce(lander, -1, self.force2, [0, 0, 0], flags=self._p.LINK_FRAME)
    self._p.applyExternalForce(lander, -1, self.force3, [0, 0, 0], flags=self._p.LINK_FRAME)
  ...
  def RGB_camera(self):
    ...
    RGB_img = self._p.getCameraImage(128, 128, view_matrix, projectionMatrix)
  ...
  def getObservation(self):
    ...
    lin_vel, ang_vel = get_base_velocity()
    RGB_img, _, _, = RGB_camera()
    lidar_results = Lidar()
    
    observation.extend(list(pos))
    observation.extend(list(orn))
    observation.extend(list(lin_vel))
    observation.extend(list(ang_vel))
  ...
  def applyAction(self, thrusterCommands):
    ...
    self._p.applyExternalForce(self.landerUniqueId, -1, self.force1, [0, 0, 0], flags=self._p.LINK_FRAME)
    self._p.applyExternalForce(self.landerUniqueId, -1, self.force2, [0, 0, 0], flags=self._p.LINK_FRAME)
    self._p.applyExternalForce(self.landerUniqueId, -1, self.force3, [0, 0, 0], flags=self._p.LINK_FRAME)
