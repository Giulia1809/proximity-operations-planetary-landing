...
class LanderGymEnv(gym.Env):
  ...
  def _reward(self):
   ...
   #TODO: Add here ad-hoc reward for velocities/pin-point landing/fuel consumption:
    ...
    #Pin-point reward:
    base_pos, _ = self._p.getBasePositionAndOrientation(self._lander.landerUniqueId) 
    x_cube = base_pos[0]
    y_cube = base_pos[1]
    spotpos = self._spotStartPos
    #print('spot pos in reward fct : ', spotpos[0:2]) #just a check
    x_spot = spotpos[0]
    y_spot = spotpos[1]
    #min_radius = 3.0
    med_radius = 5.0
    #if (abs(x_cube - x_spot) < min_radius and abs(y_cube - y_spot) < min_radius):
    #  reward_pin_point = +200
    #elif (abs(x_cube - x_spot) < med_radius and abs(y_cube - y_spot) < med_radius):
    #  reward_pin_point = +100
    #else:
    #  reward_pin_point = -200
    reward_pin_point = 200 - med_radius*np.sqrt(abs(x_cube - x_spot)**2 + abs(y_cube - y_spot)**2)
    #print('reward pin point = ', reward_pin_point)
    
    #Fuel consumption constraint:
   # _, _, _, realaction, _ = self.step(action)
    #if realaction > 5:
      #fuel_cons_reward = -25 
    #else: 
      #fuel_cons_reward = +25
     
    reward = reward_1 + reward_2 + reward_pin_point #+ fuel_cons_reward #+ reward_3
    #print("reward: ", reward)
