#Run this script to interact with Lander in the chosen Environment - use arrows to activate Thrusters. 
#This script includes sensors: RGB camera, Depth camera, Segmentation Mask, LiDAR (+ inputs from position and velocity)
...
p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4, "lunar_lander.mp4") #Record video. Comment to undo.
 
p.setAdditionalSearchPath(pybullet_data.getDataPath()) #optionally
p.setGravity(0,0,-1.62) #gravity set along -z
...
while True:
  ...
  def thruster_forces(baseForce, force1Diff, force2Diff, force3Diff, force4Diff):    
    ...
    p.applyExternalForce(boxId, -1, force1, [.25, 0, 0], flags=p.LINK_FRAME)
    p.applyExternalForce(boxId, -1, force2, [0, .25, 0], flags=p.LINK_FRAME)
    p.applyExternalForce(boxId, -1, force3, [-.25, 0, 0], flags=p.LINK_FRAME)
    p.applyExternalForce(boxId, -1, force4, [0, -.25, 0], flags=p.LINK_FRAME)
    return force1, force2, force3, force4
  force1, force2, force3, force4 = thruster_forces(baseForce, force1Diff, force2Diff, force3Diff, force4Diff)
